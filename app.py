...
def get_prediction(img_bytes,model):
    ...
    # inference
    results = model(new_img, size=640)  
    return results
...
# post method
@app.route('/getImgPrediction', methods=['POST'])
def predict_img():
    file = extract_img(request)
    img_bytes = file.read()
...
def predict_json():
    ...
    response = make_response(results.pandas().xyxy[0].to_json())
